...
% For an equally spaced grid, Lagrange's equation (or Poisson's equation
% of heat conduction at steady state if you prefer), approximated using
% finite differences to the partial derivatives, implies that at any node
% in the grid, we could replace it by the average of its 4 neighbors.
% Thus the two NaN elements generate two linear equations:
%
...
% Neat hah? For an arbitrary number of NaN elements in an array,
...
% Method 3 is yet another variation on this theme, except the
% differential equation model used is one more suited to a model
% of a thin plate than for heat diffusion. Here I've used del^4
% as the governing PDE, again discretized into a linear system of
% equations.
