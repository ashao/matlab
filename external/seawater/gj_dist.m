...
%         and N=2, or LAT=0...
%
%       Sabine Mecking 19/Dec/98
%          B missing for 'wsgi84 ?????' Added B according to folrmula given in
%          comments
%          changed name from dist.m gj_dist.m
%          
...
[mlat,nlat] = size(lat);
if mlat~=1 & nlat~=1
   error('sw_dist.m: lat, long must be vectors.  No matrices allowed')
else
  if mlat == 1
...
lat = lat(:)*pi/180;   % force to column vectors
long = long(:)*pi/180; % and convert to radians
