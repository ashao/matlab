function Dataset = nc_getvarinfo_tmw(ncfile,arg1,arg2)
% TMW backend for NC_GETVARINFO
%
% DINFO = NC_GETVARINFO_TMW(ncfile,ncid,varid);
% DINFO = NC_GETVARINFO_TMW(ncfile,ncfile,varname);
...
if ischar(arg1) && ischar(arg2)
    ...
    try
        ...
        Dataset = get_varinfo(ncfile,ncid,varid);
    catch me
        netcdf.close(ncid);
        rethrow(me);
    end
...
elseif isnumeric ( arg1 ) && isnumeric ( arg2 )
	...
	Dataset = get_varinfo(ncfile,arg1,arg2);
...
%-------------------------------------------------------------------------
function Dataset = get_varinfo(ncfile,ncid,varid)
...
if natts == 0
	...
	for attnum = 0:natts-1
		Dataset.Attribute(attnum+1) = nc_getattsinfo_tmw(ncfile,ncid,varid,attnum);
	end
end
