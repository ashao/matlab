...
infile.pden = '/home/ashao/uw-apl/models/HIM/hindcast/surface.pden.mat';
...
%%
him.so.pden = him.pden(:,him.so.sidx);
him.so.wts = metrics.Ah.data(him.so.sidx)';
him.so.wts = repmat(him.so.wts,[length(him.sam.tidx) 1])./sum(him.so.wts(:));

%%
[ntime nhimpts] = size(him.so.pden(him.sam.tidx,:));
% him.so.pden_filt = him.so.pden;

him.so.pden_filt=detrend(him.so.pden(him.sam.tidx,:)')';
for i=1:nhimpts
    if mod(i,500)==0
    fprintf('HIM %d/%d\n',i,nhimpts);
    end
%     him.so.sst_filt(:,i) = annual_harms(him.so.sst(:,i),him.time(him.so.tidx),6,0,365.25);
    ...
    for t=1:12
            tidx = t:12:ntime;
            him.so.pden_filt(tidx,i) = him.so.pden_filt(tidx,i)-mean(him.so.pden_filt(tidx,i));
    end
end
him.so.pden_filt = him.so.pden_filt.*him.so.wts;
...
%%
R_xy = double(ncep.so.slp_filt'*him.so.pden_filt./(ntime-1));
...
himfillmat(him.so.sidx)=Reofs(:,mode);
...
m_pcolor(metrics.geolon.data,metrics.geolat.data,himfillmat)
shading flat
...
colorbar
title('Mixed Layer Density Mode 1')

subplot(3,4,[3 4 7 8])
% ncepfillmat = nan(size(ncep.so.slp));
ncepfillmat=reshape(Leofs(:,mode),size(latgrid));

% m_contourf(ncep.so.lon,ncep.so.lat,ncepfillmat,levels,'LineColor','None')
m_pcolor(ncep.so.lon,ncep.so.lat,ncepfillmat)
shading flat
m_coast('line','LineWidth',2,'Color','Black');
% caxis([-1 1]*2.5e-2)
m_grid;
...
plot((1:720)/12+1948,pcR_norm,'LineWidth',1.5,'Color','Black')
% plot(1:720/12+1948,polyval(polyfit(him.time/365+1948,pcR_norm,1),him.time/365+1948))
% datetick('KeepTicks')
ylim([-3 3])
xlim([1940 2020])
grid on
...
plot((1:720)/12+1948,pcR,'LineWidth',1.5,'Color','Black')
% datetick('KeepTicks')
ylim([-3 3])
grid on

timeidx = ncep.time > datenum(1948,1,1)-1 & ncep.time < datenum(2008,1,1);
...
plot(sam_ncep.time,sam_ncep.pc./std(sam_ncep.pc),'--')
plot(ncep.time(timeidx),pcL(timeidx),'LineWidth',1.5,'Color','Black')
ylim([-3 3])
datetick('KeepTicks')
grid on

max(abs(xcov(pcL(timeidx),pcR,'coeff')))